...
void send_to_one_clients(ClientList *np, char tmp_buffer[],int daat1)
{
  printf("\n\nreached here again %d\n",daat1);

    ClientList *tmp = root->link;
    while (tmp != NULL) {
        // if ((np->data != tmp->data)&&(np->data == daat1))
        if ((tmp->data == daat1))
        {
...
void client_handler(void *p_client) {
    ...
    while (1) {
        ...
        recv(np->data , &daat , sizeof(int),0);
        if (receive > 0) {
            if (strlen(recv_buffer) == 0) {
                continue;
            }
            sprintf(send_buffer, "%s：%s from %s", np->name, recv_buffer, np->ip);
            // tesmp[0] = recv_buffer[strlen(recv_buffer)-1];
            // daat = atoi(recv_buffer[strlen(recv_buffer)-1]);
            // daat = atoi(tesmp);
        ...
        // send_to_all_clients(np, send_buffer);
        send_to_one_clients(np, send_buffer,daat);
    }
